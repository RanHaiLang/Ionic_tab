...
import {AddPutongweixiuMessagePage} from "./add-putongweixiu-message/add-putongweixiu-message";

/**
 * Generated class for the JiuzhengxingweixiugongdanListPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-jiuzhengxingweixiugongdan-list',
  templateUrl: 'jiuzhengxingweixiugongdan-list.html',
})
export class JiuzhengxingweixiugongdanListPage {

  jiuzhengweixiuList:any=[];
  username:string;
  uscudfchar05:string;
  ...
  getJzxwxList(){
    ...
    this.httpService.get(url).subscribe((res)=>{
      let data = res.json();
      this.jiuzhengweixiuList = data['result']
      ...
      setTimeout(() => {
        this.httpService.get(url).subscribe((res)=>{
          ...
          for(let i=0;i<data['result'].length;i++){
            this.jiuzhengweixiuList.push(data['result'][i])
          }
        })
  ...
  //获取详细信息
  goJiuzhengxingweixiu(evt_code,key){
    if(key=='select'){
      ...
      this.navCtrl.push(AddPutongweixiuMessagePage,{
        jzxwxDetail:[]
      })
    }
  }
  ionViewDidLoad() {
    console.log('ionViewDidLoad JiuzhengxingweixiugongdanListPage');
  }
